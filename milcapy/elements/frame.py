...
class ElasticTimoshenkoBeam3D(Element):
    ...
    def __init__(
        ...
        # Cargas =============================
        self.load: FullLinearDistributedLoad | None = None   # Cargas aplicadas

        # Ensamblaje =========================
        self.dofs: NDArray | None = None   # [ux, uy, uz, rx, ry, rz] * 2

        # Segmentos ==========================
        self.seg_axial: BarSeg | None = None
        self.seg_transverse_y: BeamSegTimoshenko | None = None
        self.seg_transverse_z: BeamSegTimoshenko | None = None
    ...
    @staticmethod
    def FEFM_timoshenko(length: float, phi: float, qi: float, qj: float) -> NDArray:
        """Retorna las fuerzas de fijacion en un pano de la viga de Timoshenko"""
        ...
        FYi = by * L / 2 + ay * L**2 / 60 * ((10 * phi + 9) / (phi + 1))
        FYj = by * L / 2 + ay * L**2 / 60 * ((20 * phi + 21) / (phi + 1))

        MZi = by * L**2 / 12 + ay * L**3 / 120 * ((5 * phi + 4) / (phi + 1))
        MZj = by * L**2 / 12 + ay * L**3 / 120 * ((5 * phi + 6) / (phi + 1))

        return np.array([FYi, FYj, MZi, MZj])




class BeamSegTimoshenko():
    ...
    def slope(self, x):
        ...
        theta_x = 1/EI * (C3 + C2*x + (C1/2)*x**2 - (B/6)*x**3 - (A/24)*x**4 + ((EI/G*As)*((A/2)*x**2 + B*x)))
...
class BarSeg():
    """
    Un segmento de barra matemáticamente continuo
    """
...
def deformed_shape(model: 'Model', n: int, escale: float, ax: 'Axes3D') -> None:
    ...
    for member in model.members.values():
        if member.model_type == ElementModelType.ElasticTimoshenkoBeam3d:
            # Segmentos
            member.seg_axial = BarSeg()
            member.seg_transverse_y = BeamSegTimoshenko()
            member.seg_transverse_z = BeamSegTimoshenko()

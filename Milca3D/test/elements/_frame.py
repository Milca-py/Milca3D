...
from milcapy.load import DistributedLoad
...
class ElasticTimoshenkoBeam3D(Element):
    ...
    def set_up(self):
        ...
        self.Tlg = transform_matrix(self)
        ...
        self.kl = local_stiffnes_matrix(
            self.E, self.G, self.A, self.length, self.Jx, self.Iy, self.Iz, self.phiY, self.phiZ)

        if self.load is not None:
            self.ql0 = q_phi(self.length, self.phiY, self.phiZ, self.load.p_i,
                             self.load.p_j, self.load.q_i, self.load.q_j, self.load.w_i, self.load.w_j)
...
def local_stiffnes_matrix(E: float, G: float, A: float, L: float, Jx: float, Iy: float, Iz: float, phiY: float, phiZ: float) -> NDArray:
...
def transform_matrix(member: ElasticTimoshenkoBeam3D) -> NDArray:
    ...
    # z' = x' × v_aux
    z_axis = np.cross(x_axis, member.v_xz)
    ...
    member.z_axis = z_axis

    return T


def q_phi(L: float, phiY: float, phiZ: float, pi: float, pj: float, qi: float, qj: float, wi: float, wj: float) -> NDArray:
    ...
    MZj = -(by * L**2 / 12 + ay * L**3 / 120 * ((5 * phiZ + 6) / (phiZ + 1)))
...
#         # AXIAL:
#         ux = np.array([member.seg_axial.displacement(xi) for xi in x]) * escale
...
#         # plt.figure()
#         # plt.plot(x, ux, label='Axial')
#         # plt.plot(x, uy, label='Transverse Y')
#         # plt.plot(x, uz, label='Transverse Z')
#         # plt.legend()

#         # plotear la forma deformada (en 3D):
#         # ax.plot([0, member.length], [0, 0], [0, 0])

#         # print(f"ux: {ux/escale}")
#         # print(f"uy: {uy/escale}")
#         # print(f"uz: {uz/escale}")
#         # print(f"ul: {member.ul}")

#         # plotear la forma original:
#         # ax.plot([member.ul[0]*escale, member.length + member.ul[6]*escale], [member.ul[1]*escale, member.ul[7]*escale], [member.ul[2]*escale, member.ul[8]*escale])
#         # ax.plot(x + ux, uy, uz, color="#0000ff")
...
#         # coord_rig_i = nd1 + [member.node_i.displacements[0]*escale, member.node_i.displacements[1]*escale, member.node_i.displacements[2]*escale]
#         # coord_rig_j = nd2 + [member.node_j.displacements[0]*escale, member.node_j.displacements[1]*escale, member.node_j.displacements[2]*escale]

#         # tranformar coordenadas de la deforma local a global: rotar y trasladar
#         lf, lo, uxi = member.length + \
#             member.node_j.displacements[0] - \
#             member.node_i.displacements[0], member.length, member.node_i.displacements[0]
#         XX = lf/lo * (x + uxi)
#         disp_shap = np.array([x + ux, uy, uz])
#         # disp_shap = np.array([XX, uy, uz])
#         disp_shap = np.dot(Tlg.T, disp_shap)
#         disp_shap = disp_shap + nd1[:, np.newaxis]

#         # ax.plot([coord_rig_i[0], coord_rig_j[0]], [coord_rig_i[1], coord_rig_j[1]], [coord_rig_i[2], coord_rig_j[2]], color="#0000ff")
